...
Truncate.pl 400 < ${stub}_U.fa > ${stub}_T400.fa
cropF.py  ${stub}_T400.fa $cropFL > ${stub}_T400_P_BC.fa

echo "Running SeqDist"
mpirun $mpiextra -np $nodes SeqDist -in ${stub}_T400_P_BC.fa > ${stub}_T400_P_BC.seqdist
xs=$?
if [[ $xs != 0 ]]; then
...
FCluster -in ${stub}_T400_P_BC.seqdist -out ${stub}_T400_P_BC_S > ${stub}_T400_P_BC.fcout
xs=$?
if [[ $xs != 0 ]]; then
...
mpirun $mpiextra -np $nodes SeqNoise -in ${stub}_T400_P_BC.fa -din ${stub}_T400_P_BC.seqdist -lin ${stub}_T400_P_BC_S.list -out ${stub}_T400_P_BC_s30_c08 -s 30.0 -c 0.08 -min ${stub}.filtered.map > ${stub}_T400_P_BC_s30_c08.snout
xs=$?
if [[ $xs != 0 ]]; then
...
ln -sf ${stub}_T400_P_BC_s30_c08_cd.fa ${stub}_F.fa
