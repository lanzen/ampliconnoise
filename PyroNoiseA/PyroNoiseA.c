...
#include "PyroNoiseA.h"

/*global constants*/
static char *usage[] = {"PyroNoiseA - clusters flowgrams without alignments\n",
...
void readData(char* szDataFile, t_Flows* ptFlows, t_Params *ptParams)
{
  ...
  if(ifp){
    ...
    szTok = strtok(NULL,DELIM);
    if(szTok != NULL){
	int m = 0, n = 0, mLen = strlen(szKey);
    	szFlows = strdup(szTok);
	
	gOffSet = 0;
	while(m < mLen){
	  if(szKey[m] == szFlows[n]){
	    m++;
	  }
	  n++;
	}
	gOffSet = n;
      ...
      szFlows = (char *) malloc((nM+1)*sizeof(char));
      for(j = 0; j < nM; j = j+4){
	strcat(szFlows,"TACG");
      }
    }
...
char* flowToSeq(int *pnSeqLength, double *adFlows, int nLength)
{
  ...
  for(i = gOffSet; i < nLength; i++){
    char cBase = szFlows[i];
...
void writeClustersF(t_Params *ptParams, int nN, int nM, int nK, double *adWeight, short* asCentroids)
{
  ...
  for(i = 0; i < nK; i++){
    if(adWeight[i] > 0){
      ...
      for(j = gOffSet; j < nM; j++){
	char   cBase = szFlows[j];
