...
int main(int argc, char* argv[]){
  ...
  if(rank == 0){//head node reads data etc.
    ...
    while((nIter < MIN_ITER) || ((dMaxDelta > MIN_DELTA) && (nIter < MAX_ITER))){
      ...
      calcCentroidsMaster(anChange, 0, nA0, &tFlows, &tMaster, anCentroids, &tUnique);
	...
	MPI_Recv(&anChange[nStart], nA, 
      ...
      calcDistancesMaster(adDist, anChange, adNorm, &tMaster, dSigma, anCentroids, &tFlows, adWeight, nI0,&tUnique);
	...
	anT[anZ[i]]++;
      ...
      calcCentroidsMaster(anChange, 0, nK, &tFlows, &tMaster, anCentroids, &tUnique);
	...
	goto finish;
      ...
      calcCentroidsMaster(anChange, nStart, nFinish, &tFlows, &tMaster, anCentroids, &tUnique);
      ...
      calcDistancesSlave(adDist, nM, nK, anChange, adLocalNorm, &tLetter, dSigma,
			      anCentroids, &tFlows, adWeight, nIStart, nIFinish, &tUnique);
...
void calcCentroidsMaster(int *anChange, int nKStart, int nKFinish, t_Flows *ptFlows, t_Master *ptMaster, int *anCentroids, t_Unique *ptUnique)
{
  ...
  for(i = nKStart; i < nKFinish; i++){ //loop clusters
	...
	for(k = 0; k < nL; k++){
	  double dDistX =  alignX(&asU[anL[k]*nM], asFlows, nM, anLengths[nI]);
			 ...
			 int* anCentroids, t_Flows *ptFlows, double* adWeight, int nIStart, int nIFinish, t_Unique *ptUnique)
{
  ...
  for(i = nIStart; i < nIFinish; i++){
	...
	adDist[nO + k] = alignX(&asU[anCentroids[k]*nM], &asData[nM*i], nM, anLengths[i]);
      }
...
{
  int k = 0, i = 0, nK = ptMaster->nK, nM = ptMaster->nM;
  double adTau[nK];
  int *anLengths = ptFlows->anLengths;
  short *asData  = ptFlows->asData;
  short *asU     = ptUnique->asU;
  ...
  for(i = 0; i < nI0; i++){
    ...
    for(k = 0; k < nK; k++){
      if(adWeight[k] > MIN_WEIGHT && anChange[k] == TRUE){
	adDist[i*nK + k] = alignX(&asU[anCentroids[k]*nM], &asData[nM*i], nM, anLengths[i]);
      }
