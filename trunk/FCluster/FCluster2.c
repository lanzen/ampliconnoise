...
static char *usage[] = {"FCluster\n",
			...
			"Options:\n",
			"-wf                      string separate weights file\n",
			"-r                       resolution\n",
			"-a                       average linkage\n",
			"-w                       use weights\n",
			"-i                       read identifiers\n",
...
int main(int argc, char* argv[])
{
  ...
  if(tParams.szWeightsFile != NULL){
	readWeightsFile(tParams.szWeightsFile, nN, afW);
	tParams.bWeight = TRUE;
  }
...
void getCommandLineParams(t_Params *ptParams,int argc,char *argv[])
{
  ...
  /*get output filestub*/
  ptParams->szWeightsFile = NULL;
  ptParams->szWeightsFile  = extractParameter(argc,argv,WEIGHTS_FILE, OPTION);  
...
void readDistanceMatrix(char*** paszID, char *szDistFile, float *** paafDist, int *pnN, t_Params *ptParams, float** pafW)
{
  ...
  if(ifp){
    ...
    for(i = 1; i < nN; i++){
      ...
      aafDist[i] = (float *) malloc(i*sizeof(float));
      if(!aafDist[i])
	goto memoryError;
...
void readWeightsFile(char *szWeightsFile, int nN, float* afW)
{
 FILE *ifp = NULL;
 char szLine[MAX_LINE_LENGTH];
 char *szTok = NULL, *pcError = NULL, *szBrk = NULL;
 int  i = 0, j = 0;
  ...
  if(ifp){
	for(i = 0; i < nN; i++){
		...
		szBrk = strpbrk(szLine, "\n");
		(*szBrk) = '\0';
		afW[i] = (float) getWeight(szLine);
      	}

    fclose(ifp);
  }
  else{
    fprintf(stderr, "Failed to open file %s in readDistanceMatrix",szWeightsFile);
    fflush(stderr);
    exit(EXIT_FAILURE);
  }
